...
export async function login(req, res) {
    try {
        ...
        // Find chef by email address
        const chef = await ChefModel.findOne({ emailAddress });
        ...
        // Generate token
        const token = jwt.sign({ chefId: chef._id }, process.env.JWT_SECRET, { expiresIn: '1h' });

        // Set token in cookie
        res.cookie('token', token, { httpOnly: true, maxAge: 3600000 }); // Token expires in 1 hour

        // Respond with user data and token
        res.status(200).json({ chef, token });
    } catch (error) {
...
export async function forgotPassword(req, res) {
    try {
        ...
        // Update chef with reset token and expiry time
        chef.resetPasswordToken = resetToken;
        chef.resetPasswordExpires = Date.now() + 3600000; // Token expires in 1 hour
        await chef.save();

        // Send password reset email
        await sendPasswordResetEmail(chef, resetToken);
...
export async function resetPassword(req, res) {
    try {
        ...
        // Find chef by reset token
        const chef = await ChefModel.findOne({ resetPasswordToken: resetToken });
        console.log("chef", chef);
        ...
        if (chef.resetPasswordExpires < Date.now()) {
        ...
        // Update chef's password and clear reset token
...
// Method to change chef password
export async function changePassword(req, res) {
    try {
        ...
        const chefId = decodedToken.chefId;

        // Find the chef by chefId
